...
def find_min_T(N, M, D, U, adjacency, start_positions, end_positions):
    # ── Algebraic pre-checks (O(1), no Z3 needed) ────────────────────────────

    # Pigeonhole: minimum occupancy per station is ⌈N/M⌉; if that exceeds D,
    # no schedule exists at any T.
    if ceil(N / M) > D:
        return {
    ...
    # ── Incremental solver + unsat-core early exit ────────────────────────────
    ...
    # Build permanent base constraints up to T_lower (encode once, extend later)
    ...
    for _ in range(T_lower, 51):
        # Try endpoint pins at t = T_current-1 as soft assumptions
        ...
        # Unsat-core early exit: if no endpoint assumption is in the core,
        # the infeasibility is structural — more T won't help.
        core = s.unsat_core()
        s.pop()
        ...
        # Extend base by exactly one timestep
        new_t = T_current
